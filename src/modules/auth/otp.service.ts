...
export class OtpService {
  ...
  async verifyOtpCode(phone_number: string, code: string) {
    ...
    const token = 'generated-success-token';

    await this.setSuccessToken(phone_number, token);
  ...
  async setSuccessToken(phone_number: string, token: string) {
    const key = `sms:otp:${phone_number}:token`;
    await this.redisService.addKey(key, token, 300);
  }

  async checkSuccessToken(phone_number: string) {
    const key = `sms:otp:${phone_number}:token`;
    const value = await this.redisService.getKeyValue(key);
